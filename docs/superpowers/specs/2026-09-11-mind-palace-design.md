...
卡片墙。每张卡：群名、活跃天数、观点数、多空倾向条、常聊板块 chips、讨论股票数、最近活跃时间。支持按活跃度/观点数/股票数排序与名称搜索。

### 9.2 `/kol/:chatId` 大V详情

- **顶部风格画像**：多空倾向、操作风格标签、覆盖广度、活跃时段、常聊板块。
- **主体股票列表**：代码/名称、提及次数、多空、最近操作标签、最近提及时间；排序切换（提及次数 / 最近）。
- **点开股票 → 观点时间线**：按时间倒序，含原文，标出多空与操作标签。

### 9.3 `/stock/:code` 扩展

在现有 `StockDetail.tsx` 增加「大V观点」区块：哪些大V讨论过该票、各自的操作/多空/最近时间，可展开看该大V对这只票的观点时间线（复用 `GET /api/palace/kols/{chat_id}/stocks/{code}`）。

**这是本期唯一改动的现有页面**，改动限定为「新增一个区块」，不动既有逻辑与布局。

### 9.4 导航

`Navbar` 新增「大V」入口指向 `/kols`。`App.tsx` 新增两条路由，沿用现有 `Suspense` + 懒加载写法。

## 10. 更新机制

- `build_palace.py` 接在采集流程之后（或 cron 每日一次）：增量读档案新增部分 → 抽取观点 → 重建两个索引。
